...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        tgt2 = self.self_attn(
            q.transpose(0, 1), k.transpose(0, 1), tgt.transpose(0, 1), attn_mask=tgt_mask
        )[0].transpose(0, 1)
...
class OVDeformableTransformer(nn.Module):
    ...
    def forward(
        ...
        if self.two_stage:
            ...
            num_patch = len(text_query)
            query_embed = query_embed.repeat(num_patch, 1)
            query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1)
            tgt = tgt.repeat(num_patch, 1)
        ...
        return (hs, init_reference_out, inter_references_out, None, None, cache), memory_features
