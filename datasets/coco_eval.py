...
class CocoEvaluator:
    def __init__(self, coco_gt, iou_types, cat2label=None, label_map=False, unseen_list=None):
        """
        Args:
            coco_gt: Ground truth COCO object.
            iou_types: List or tuple of IOU types (e.g., ['bbox', 'segm']).
            cat2label: Optional dictionary for mapping categories to labels.
            label_map: Whether to use category mapping.
            unseen_list: Optional list of unseen category IDs (if any).
        """
        ...
        self.label2cat = {v: k for k, v in cat2label.items()} if cat2label else None
        self.label_map = label_map
        self.unseen_list = unseen_list if unseen_list is not None else []
                ...
                [
                    {
                        ...
                        "category_id": self.label2cat[labels[k]] if self.label_map and self.label2cat else labels[k],
                        "bbox": box,
                        "score": scores[k],
                    }
    ...
    def prepare_for_coco_segmentation(self, predictions):
        ...
        for original_id, prediction in predictions.items():
            ...
            masks = prediction["masks"].cpu() > 0.5
...
def evaluate(self):
    ...
    if p.iouType in ["segm", "bbox"]:
        ...
        raise ValueError("Unknown IoU type specified: {}".format(p.iouType))
